...
def read_DROZY_videos(flag_drowsiness, input_file, min_difference):
    detector = dlib.get_frontal_face_detector()
    predictor = dlib.shape_predictor('shape_predictor_68_face_landmarks.dat')
    print('input file : ')
    print(input_file.split('.mp4')[0] + '.txt')
    frames_timestamp = get_frames(input_file.split('.mp4')[0] + '.txt', min_difference)
    frame_counter = 0
    cap = cv2.VideoCapture('Data/DROZY/videos_i8/' + input_file)
    print(int(cap.get(cv2.CAP_PROP_FRAME_COUNT)))
    while cap.isOpened():
        _, frame = cap.read()
        if frame_counter in frames_timestamp:
            image = frame
            ...
            for (i, rect) in enumerate(rects):
                ...
                image, lip_first_layer_distance_1, lip_first_layer_distance_2, lip_first_layer_distance_3, lip_first_layer_distance_4, lip_first_layer_distance_5, lip_second_layer_distance_1, lip_second_layer_distance_2, lip_second_layer_distance_3 = lips_distance(
                    shape, image)
            distances = [frames_timestamp[frame_counter]['time'], flag_drowsiness, eye_left_distance_1,
            ...
            if frame_counter == list(frames_timestamp.keys())[-1]:
                return 0
        frame_counter += 1
